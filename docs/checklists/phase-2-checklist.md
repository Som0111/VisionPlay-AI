...
- [ ] `vision/inference/backend_manager.py` — `BackendManager`: constructs and owns
...
- [ ] `BackendManager` registration: a factory mapping from backend name to a constructor
...
- [ ] Configuration: extend `core/config.py`'s per-namespace sections with an `inference`
...
- [ ] Capability representation: a way to ask "is backend X available right now" — checks
...
- [ ] `BackendManager` never gets called by plugins directly — only the pipeline (M2.3)
      constructs and queries it, preserving the "plugins never instantiate backends" rule from
      `docs/architecture.md` §3/§4.
- [ ] Unit tests: registering a backend factory and resolving it by name; requesting an
...
- [ ] `vision/inference/mediapipe_backend.py` (modify) — `MediaPipeBackend.load()` builds the
...
- [ ] `vision/inference/onnx_backend.py` (modify) — `ONNXBackend.load()` constructs
...
- [ ] `vision/inference/model_registry.py` (modify) — implement a real `ModelDownloader`
      ...
      unit tests.
- [ ] Model loading/caching: confirm `ModelRegistry.ensure()`/`is_cached()` are called from
      ...
      `infer()`.
- [ ] Standardized inference outputs: a small set of result value objects (e.g. hand/pose
...
- [ ] Unit tests: `MediaPipeBackend.infer()` returns the standardized landmark output for a
...
- [ ] `vision/pipeline/frame_bus.py` (modify) — replace the M1.4 no-op backend step: for the
...
- [ ] `frame.results` is populated in place, keyed by each backend's `name` property (e.g.
...
- [ ] `FramePipeline` integration: backends declared by the active app are loaded on app start
      ...
      per frame.
- [ ] Capability negotiation: `ui/launcher/launcher_widget.py` (modify) — an app's
...
- [ ] Preserve existing plugin API compatibility: no changes to `AppPlugin`, `AppManifest`, or
...
- [ ] A backend `load()`/`infer()` failure is contained the same way plugin exceptions are
...
- [ ] Unit/integration tests: with a fixture plugin declaring a real backend, `frame.results`
...
- [ ] Performance: confirm per-frame backend cost stays bounded under the existing
      ...
      just fixture images.
- [ ] Error handling: every backend failure path (missing model file, checksum mismatch,
...
- [ ] Unit & integration tests: full headless coverage for `BackendManager`, both real backend
...
- [ ] Documentation: update `docs/architecture.md` §5 to reflect that CPU-only MediaPipe/ONNX
      ...
      `model_registry.py`.
- [ ] Production-ready cleanup: no remaining `NotImplementedError` stubs in the inference
...
- [ ] All milestones M2.1–M2.4 complete.
- [ ] `BackendManager` constructs, shares, and warm-caches real `MediaPipeBackend`/`ONNXBackend`
      instances by name, driven entirely by configuration and each app's `required_backends`.
- [ ] `MediaPipeBackend` (hand landmarks) and `ONNXBackend` run real CPU-only inference and
...
- [ ] The launcher greys out apps whose `required_backends` aren't satisfied instead of letting
      them crash at launch; a satisfied app launches and runs real inference end-to-end through
      `FramePipeline`.
- [ ] The hand-tracking demo app (Phase 1's headline deliverable) now renders real hand
...
- [ ] No plugin API redesign: `core/plugin_base.py` is unchanged from Phase 1; no gesture
      recognition or game logic was added to any app (reserved for Phase 3).
- [ ] v1 remains CPU-only — no `onnxruntime-gpu`, no CUDA/DirectML, no GPU-specific code path;
      `DeviceConfig`/execution-provider lists are unchanged in shape from Phase 0, only
      exercised for real.
- [ ] `ruff`, `mypy`, and `pytest` (including new backend/integration tests) all green in CI on
      Windows.
