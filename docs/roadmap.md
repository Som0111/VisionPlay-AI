...
| Phase | Goal |
|---|---|
| 0 — Foundation | Repo scaffold, CI skeleton, core (config/logging/paths/event bus), camera-to-Qt render pipeline proven end-to-end, **PyInstaller packaging spike**. Session-by-session breakdown: [`phase-0-checklist.md`](./phase-0-checklist.md) |
| 1 — Plugin skeleton | `AppPlugin`/`AppManifest`/registry, launcher UI, `_template` app, one trivial hand-tracking "hello world" through the full pipeline |
| 2 — Inference backends | `InferenceBackend` ABC (with a `device` param even though v1 only implements `cpu`), CPU-only MediaPipe + ONNX implementations, model registry, capability negotiation in launcher |
